...
- （2026-06-09 第九十二次）其餘 6 職務 `doc/purchase/sales/pharmacist/warehouse/qa` 的 `draftQuestions` 各再補 **3 題**政府公開查核重點題：
  - 取材自各職務 SOP 領域對應的已驗證 `focus` 缺失項（同檔內已標 TFDA/PIC/S 來源者），轉成測驗題，非新編缺失。
  - 來源字串標 `TFDA 111年度GDP實地查核流程與常見缺失`、`TFDA 113年度GDP申請資料準備與常見缺失` 或 `PIC/S PI044-1 GDP Aide-Memoire`。
  - 對應領域：文管＝SOP一致性/紀錄保存/紀錄即時填寫；採購＝供應商GDP符合性/採購紀錄追溯/委外合約；業務＝客戶認可/客訴分類/運銷紀錄；管理藥師＝模擬回收/疑似偽禁仿冒藥來源/品質風險；倉管＝溫度測繪/FEFO/設備校正；品管＝CAPA根因/內部稽核追蹤/偏差提報。
