...
### 本次完成
1. **規範 §49 新增並升版**（`GDP_智慧查詢規範.md` v3.2→v3.3）：AI 管理助理定位為管理者維護入口，不是自動上線機器人。
2. 定版四類需求分流：
   - 帳號與職務：AI 可引導填表、產生變更摘要；管理者確認後才寫 Firestore，且必須留下 `gdpAuditLogs` / `gdpRoleChangeLogs`。
   - 題庫與測驗：AI 只能建立待審核草稿，需來源文件與講師/文管/管理者確認。
   - 教材內容修正：AI 只能提出原文、建議修改、來源文件、影響區塊，不得直接覆蓋正式資料。
   - 網站功能需求：涉及 HTML/JS/Firebase rules/GitHub Pages/登入權限者，必須走 GitHub Issue / PR / 預覽 / 測試 / 核准，不得直接改正式站。
3. 補明安全邊界：AI 管理助理只給 `gdp_admin`、`doc_superadmin` 或指定管理者；不得顯示 API key、token、完整個資、未授權部門資料；所有 AI 協助變更都需可追溯。

### 重要設計決策
- AI 可以降低管理者維護門檻，但不得繞過審核。
- 日常資料異動可走平台內確認流程。
- 教材、題庫、KB 一律先草稿後審核。
- 程式功能一律 PR 化，不允許 AI 從正式站直接寫入程式或 rules。

### 下次待辦
1. **登入／正式權限（Phase 3 實作）**：仍需先確認管理員 email、超管 email、`gdp_admin` 指定人員、各角色可見範圍與 Firebase collection/rules。
2. **AI 管理助理第一版（Phase 4 / 維護層）**：等登入/權限穩定後，先做「需求分類＋草稿產生＋帳號職務異動確認」；程式功能自動 PR 流程後續再評估。
...
### 根因
- `deptQuickNav()` 永遠渲染全部 4 顆段落按鈕（資料時程／作業流程／稽查翻卡／文件地圖），但段落顯示由閱讀模式的 `data-ux-kind` token 控制（CSS L207-215）。
- 錨點所在區塊 token：資料時程＝`quick schedule`、作業流程＝`schedule`。在「稽查準備(inspection)／測驗複習(review)」模式下兩者皆 `display:none`，按鈕卻還在 → 點擊跳到隱藏錨點，瀏覽器無法捲動 → 看似「無動作」（死連結）。
- 屬模式相關：完整閱讀／文件時程模式下兩顆本來就正常。

### 修法（方案1：導覽列跟著閱讀模式走）
- `bindDeptReadModes()` 的 `apply()` 內新增 `syncQuickNav(active)`：依每顆按鈕目標所屬 `data-ux-kind` token 是否含當前模式，決定按鈕 `display`。目標被藏的按鈕一併隱藏，導覽列只留真正能跳的段落。不破壞閱讀模式精簡用意。

### 驗收（Launch 預覽 localhost:3333 實機，品保頁全 5 模式矩陣）
- 死連結 `dead:false` 全數歸零；btnShown 與 targetVisible 完全一致。
- 各模式導覽列誠實縮減：稽查準備＝稽查翻卡/文件地圖；新人快讀＝資料時程/稽查翻卡；文件時程＝資料時程/作業流程/文件地圖；完整閱讀＝4顆全在。
- 實際點擊：稽查準備點🎯、完整閱讀點📅 皆正確捲動到目標。
- `JS_PARSE_OK 1`、console 無 error。

### 下次待辦
1. 登入／正式權限（⏸️ 需使用者提供「部門×角色×可見資料」權限表）。
2. （可選）全站圓角/陰影 token、`--ink-soft` 對比、部門頁列印一頁讀書單。

---

## 2026-06-12 第一百零六次：部門頁閱讀模式＋本頁三件事＋手機快捷列 ✅

使用者依 Codex 建議執行第二點「前端閱讀／操作 UX 精修」，本輪只動 `HTML資料庫/新勝GDP資料庫.html`，不改 SOP/fact/題庫內容，不碰登入權限與 Firebase rules。

### 本次完成
- **部門頁閱讀模式**：新增 sticky「閱讀模式」列，含 `完整閱讀 / 新人快讀 / 稽查準備 / 文件時程 / 測驗複習`。以 `data-ux-kind` 控制既有區塊顯示，不重寫內容；模式會存入 `localStorage`，切頁後自動套用。切到無閱讀模式的頁面會清除 `#content.dataset.readMode`，避免舊模式誤隱藏其他頁面。
...
### 1. 對齊 git ＋補部署第103次（題庫五欄）
- 開工 `git diff HEAD` + `git log origin..HEAD` 對齊：HEAD==origin（`f57bc86`），但工作樹有第103次未提交的題庫五欄成果（HANDOFF +27、HTML 577行變動）。
- 驗證 diff 方向為**前進非漂移**：五欄各 ×284 全在 + 端、- 端 0；`JS_PARSE_OK`、`verify_facts 1296/1296`、題庫284題選項皆4。
- 清掉 23 個暫存解壓檔（`*.txt`/`*_暫存.xml`/`temp_docx/`/`_暫存_docx_extract/`），保留 4 個章節來源資料夾不動。
- commit `6425926` + push；線上實測永久網址 HTTP 200、五欄各 ×284 命中，**第103次成果正式生效**。
- 教訓：**HANDOFF 標 ✅ ≠ 已部署**；本案是「auto-save 進 git 卻沒 commit/push」同類缺口的反向實例（git 正確報 dirty，但完成工作停在工作樹）。已寫入記憶與踩坑。

### 2. 官方常見缺失 34 題 → 改民國年度來源標註（使用者裁示：免 OCR 頁碼）
- 使用者裁示「無須補充頁碼，補民國哪一年的缺失」→ 解除「本機無 OCR」卡關。
- 34 題年度加題分兩類：12 題規範類（出題規範題，本就五欄齊備、不動）；22 題政府缺失題（sourceQuote 原為「待OCR」佔位）。
- 22 題依各題 `source` 年份逐題改誠實年度標註：TFDA 113年度說明會／111年度說明會／PIC/S 國際無民國年；reviewedBy 改「Codex 依官方來源年度標註核對」、reviewDate 補 `2026-06-11`。**不杜撰原文、不補頁碼**。
- commit `48a2f4f` + push；線上實測待OCR殘留 0、年度標註命中 22，生效。

### 3. 講師簽核上架 → 使用者裁示「不需要」
- 使用者看測試專區第二頁截圖後裁示：五欄齊備即可，不再加講師逐題簽核流程。
- 依規則使用者裁示跳過項不再列為待辦，已從待辦清單移除。

### 驗收
- `JS_PARSE_OK`、`verify_facts 1296/1296`、題庫 284 題、選項皆 4、年度加題待OCR殘留 0、reviewDate 空 0。
- origin==HEAD==`48a2f4f`；永久網址線上＝本機。

### 下次待辦
1. 登入／正式權限（⏸️ 需使用者提供「部門×角色×可見資料」權限表才能動工）。

---

## 2026-06-10 第一百零三次：測驗題庫 7 職務全部逐題回 SOP 原文核對＋補五欄＋修錯題 ✅

使用者下令「逐一檢查」，並糾正前次未把 30 份來源抽齊、未逐章比對的疏失。本輪一章一章打開資料夾原文（36 份 docx 全部新鮮解壓到 TEMP），把 7 職務題庫**逐題**回 SOP 原文核對，逐題補 `explanation / sourceSection / sourceQuote / reviewedBy / reviewDate` 五欄。

### 出題標準依據（使用者裁示）
- 共同基礎＝`DM10-01` 全文 + `WI10-01 SMF` 第參(人事)「三、關鍵人員之資格、經驗及職責」所界定範圍；各職務專業 SOP 依該節逐字確認；常見缺失＝TFDA／PIC/S 官方公開。
- `reviewedBy`：源核對標記為「Codex 逐題回 SOP 原文核對」+ `reviewDate`；不假冒講師、不擅自標 active。講師正式簽核由測試專區第二頁輸入、提交後依表格顯示，**不需逐題重複簽核**（使用者裁示）。
- 弱題尺度：有 SOP／官方常見缺失等來源依據即保留，只移除完全無來源者。

### 本輪完成（`HTML資料庫/新勝GDP資料庫.html`，題庫 293→284）
- **文管 doc 50→41**：移除 6 題 DP24-01（教育訓練，非文管法定受訓範圍、roleDocs 無 DP24 屬死題）+ 2 題純測驗設計弱題 + 1 題「職務交接清冊」無明文題；**修 2 答案錯**：①「文件代碼 X/Y」實為**頁次** X/Y（DP42-01 原文 X=目前頁次、Y=全文頁數）②「文件發行前確認分發對象」與 v4(115/01/30) 已改「文管統一存放文件櫃、不個別分發」牴觸 → 改為登錄 FR10-01＋統一存放。
- **人事 30／採購 37／業務 37／管理藥師 35／品管 50**：逐題回原文核對，**答案全部正確、皆有明文，無錯題無死題**，僅補五欄。
- **倉管 54**：修 2 項 → ①死題 #1：4822「70 分合格」source 由 `DP24-01`（非倉管範圍）改 `WI10-01`（第參自身明文 70 分，屬共同基礎）②警戒值 #4：依 DP33-01 原文＋SMF（室溫 25→23.5、冷藏 2~8→3.5/6.5）改為「依儲存條件上下限各內縮 1.5℃」稽查語氣；其餘補五欄。
- **官方常見缺失題（年度加題 34 題）**：sourceQuote 標「待 #3 官方 PDF OCR 逐題核對補頁碼」、reviewedBy 標「待官方來源 OCR 逐題核對」、reviewDate 留空，誠實標示尚未源核對。

### 驗收
- 題庫結構：284 題、選項皆 4、五欄齊備 284/284、**來源越界 0**、冷鏈/冷藏字樣 0。
- `JS_PARSE_OK 1`；`verify_facts_ghpages.mjs` `1296/1296` 通過、合規掃描通過。
- 36 份 SOP 原文解壓在系統 TEMP（`$TEMP/gdp_sop`），未進 repo。

### 下次待辦
1. **官方常見缺失 34 題**：需先完成 TFDA 113／111、PIC/S PI044-1 PDF 的 OCR／文字化，再逐題補 sourceQuote 頁碼與原文摘錄（本機目前無 OCR 工具，屬 #3）。
2. 講師正式簽核上架：題庫已逐題回原文核對並附三欄來源，待講師於測試專區第二頁登載姓名、正式啟用。
3. 登入/正式權限（⏸️ 需權限表）。

---

## 2026-06-10 第一百零二次：測驗題庫逐題核對盤點＋開工文件精簡規則 ✅

使用者要求依上一輪 OpenAI 執行內容，重新逐一核對測驗平台正式上架狀態，並確認題庫是否足夠；同時明確要求開工文件不要越放越多，完成的輔助文件要清掉，只保留待辦事項與規範。

### 本次完成
1. **測驗題庫盤點**
   - HTML `examTemplates` 實際題庫共 293 題。
   - 7 職務題量：文管 50、人事 30、採購 37、業務 37、管理藥師 35、倉管 54、品管 50。
   - 每題皆為 4 選項；資料層仍採 `options[0]` 為正解，考試時洗牌。

2. **來源與抽題核對**
   - 公司 SOP/WI/FR 來源代碼 30 份皆找得到對應 docx，未發現題目指向不存在來源文件。
   - 抽題模擬：7 職務 × 新人/年度 × 各 1000 次，`SIM_FAILS 0`。
   - 新人三個月測驗不抽政府題；年度教育訓練測驗每次保留 1 題政府公開缺失題。
   - 即使扣除死題，所有職務有效題量仍足以支撐新人 10 題與年度 20 題。

3. **已核對範例**
   - 倉庫警戒值題回 `DP33-01` 原文，確認有「設定溫度上下限警戒值為儲區規範溫度儲存條件減 1.5℃」。
   - 品質紀錄最低保存年限回 `DP42-02` 原文，確認「至少 5 年」。
   - 委外廠商 60 分以上處置回 `DP72-01` 原文，確認「暫停交易」。
   - 確效報告保存回 `DP36-01` 原文，確認「交由品保保存 5 年」。

### 發現問題
1. **正式上架欄位不足**
   - 293 題全部缺 `reviewedBy`、`reviewDate`、`explanation`、`sourceSection`、`sourceQuote`。
   - 結論：目前仍是草稿 / 準正式題庫，不可直接標為正式啟用。

2. **7 題為抽題死題**
   - 文管 #9-#14 來源為 `DP24-01`，但文管 roleDocs 只有 `DP42-01 / DP42-02`，共同基礎只有 `DM10-01 / WI10-01`，所以抽題邏輯不會抽到。
   - 倉管 #4 來源為 `DP24-01`，但倉管 roleDocs 不含 DP24；此題可由 `WI10-01` 合格標準支撐，正式版應改來源或納入共同基礎。

3. **官方缺失題尚不能逐頁蓋章**
   - 官方常見缺失題共 34 題；官方 PDF 檔存在，但本機目前無法直接抽出可逐題核對的 PDF 內文。
   - 正式上架前需 OCR 或可抽文字版本，再補來源段落 / 頁碼 / 短摘錄。

4. **倉庫警戒值題建議微調**
   - 現答案「儲區規範溫度儲存條件減 1.5℃」有來源，但為避免誤會，正式版建議改成「依儲存條件上下限內縮 1.5℃作為警戒值」。

### 文件精簡規則
- 不再新增 `OPENAI_開工檢查指導.md` 這類輔助開工文件；本輪已刪除該臨時文件，未留下新開工文件。
- 開工 / 收工只更新既有權威文件：`HANDOFF.md`、必要規範、第二大腦。
- `HANDOFF.md` 最上方保留最新可接狀態；舊輪次僅作歷史，不得把舊待辦重新列為現況。

### 下次待辦
1. 測驗題庫正式上架前，先處理 7 題抽題死題（移除、改來源，或明確納入共同基礎）。
2. 逐題補 `reviewedBy / reviewDate / explanation / sourceSection / sourceQuote`。
3. 官方常見缺失題需先完成 PDF OCR / 文字化，再逐題補來源段落。
4. 倉庫警戒值題改為「上下限內縮 1.5℃」的稽查回答語氣。
5. 登入/正式權限仍需權限表；Firebase 題庫、成績保存、文管列印包須等權限設計一起定案。

---

## 2026-06-10 第一百零一次：外部明文與公司 SOP 自動核對規則制度化 ✅

使用者釐清「PIC/S 明文但公司 SOP 漏列項目盤點」不是獨立待辦，而是 agent 在製作資料時必須同步執行的自動比對。

### 本次完成
1. **更新 `GDP_智慧查詢規範.md` 至 v3.1**
   - 新增「外部明文與公司 SOP 的自動核對規則」。
   - 適用於：常見缺失、稽查員問法、公司回答方向、測驗題庫、fact 拆解、逐筆校對。
   - 規定外部來源有明文時，必須同步回公司 SOP/WI/FR 核對；若公司文件未明文，只能列入差異清單交使用者判斷，不得自動補成公司制度、正式題庫或新勝做法。
   - 例：常見缺失涉及倉庫溫度時，內部人員回答「15 度至 25 度」的前提，是已回公司 SOP 明文來源核對。

### 狀態修正
- 「PIC/S 明文但 SOP 漏列項目盤點」不再作為獨立待辦列出。
- 後續 agent 做常見缺失、題庫或 facts 時，若發現外部明文與公司 SOP 不一致，才產生「差異清單」給使用者判斷是否修 SOP 或僅作外部查核提醒。
...
### 本輪修正
- `HTML資料庫/新勝GDP資料庫.html`
  - 新增 `isMobileShareSurface()`：手機寬度或常見手機 WebView / App 內建瀏覽器時，不再硬走下載。
  - 新增 `openExamPacketPage()`：把送交人事 HTML 資料包以新分頁開啟，供手機直接用瀏覽器分享、列印或另存。
  - `shareExamPacket()`：若 Web Share 檔案分享不可用，不再 fallback 到下載，改開啟資料包頁面。
  - `downloadExamShareText()`：手機環境直接開啟資料包頁面，桌機才維持下載 HTML。
  - 結果頁新增「開啟送交人事資料」按鈕，讓手機使用者有明確入口。

### 驗收
- `JS_PARSE_OK 362509` 通過。
- `node verify_facts_ghpages.mjs`：`1296/1296` 通過。
- 未修改測驗題庫、答案、抽題分層、KB facts 或 SOP 內容。

### 下次注意
- 手機 App 內建瀏覽器對 `download` 與檔案分享支援不一致，送交人事資料應保留「開啟資料包」作為最穩 fallback。
- 若日後導入正式登入/Firebase attempt，手機送交應改為內部送交紀錄，不依賴下載檔。
