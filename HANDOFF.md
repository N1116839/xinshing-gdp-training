## 2026-06-04 第七十六次：文管測驗題庫擴充（20→56題）、下載改HTML、推撥改人事

依使用者要求「測試平台相關；文管20題題型不足需擴充；推撥從文管改為人事；列印上移除新勝GDP資料庫」，本輪完成。

### 本輪完成
- `HTML資料庫/新勝GDP資料庫.html`
  - **題庫從 20 題擴充為 56 題**，新增 36 題涵蓋：
    - 共同基礎（DM10-01/WI10-01）4 題：文件層級順序、品質政策所在文件、SMF 不包含項目、品質系統運作面向
    - 教育訓練（DP24-01）2 題：訓練紀錄保存五年、有效性審查會議
    - 文管專業（DP42-01）11 題：文件代碼 X/Y 含義、生效前訓練考核、定期審查十二月、審查核准時限（2 天/5 天）、修訂觸發條件、作廢文件處理、電子文件僅供查閱、申請表單 FR42-02、緊急修訂流程、發行前登錄清冊、外部文件管理、文件遺失補發
    - 文管專業（DP42-02）7 題：保存年限至少五年、填寫不得鉛筆、保存期滿延長處理、跨年度歸檔、廢止申請單 FR42-05、保存方式實體+電子、交接確認完整性
    - 綜合情境（DP42-01+DP42-02）6 題：新版文件發行確認、版本不一致代表問題、年度審查無修訂記錄、職責分工變更訓練、表單改善評估、離職交接
  - **下載送交人事資料**：`.txt` 改為 standalone `.html`，內嵌 CSS，可直接開啟瀏覽器列印，含 FR24-03 表頭、得分、題目回顧
  - **推撥文字「文管」全部改為「人事」**（7 處）：資料包標題、按鈕文字、說明文字、下載檔名、複製按鈕回饋文字
  - 頁面標題從「新勝GDP資料庫」改為「新勝醫藥 GDP 教育訓練」
...
### 下次提醒
- 題庫目前為 56 題（文管範本），其他部門題庫仍需各部門 SOP 原文逐題設計。
- 下載改為 standalone HTML 後，可直接列印成 PDF（瀏覽器 Ctrl+P）；不需要 PDF 函式庫。
- 測驗平台登入/正式權限仍未實作。
