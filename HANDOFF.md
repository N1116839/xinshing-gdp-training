...
> 最新補記：2026-06-26（第137次：**FR24-03 員工教育訓練紀錄表底部加「考試人簽名／授課講師簽名」欄 ✅**，用途＝列印後親簽證明本人應考；版本踢人 build `20260624→20260625`，commit `13e6e87` 已上線並用永久網址驗收）。
...
| 版本踢人 | 站台根目錄 `app-version.json`（`build`）＋HTML `APP_BUILD` 常數＋`versionGuard`。要強制全員重新整理（＝重新登入）＝兩處 build 一起 +1 再 push。**目前 build＝`20260625`** |
| 最新本輪修正（第137次） | FR24-03 員工教育訓練紀錄表底部新增 `.exam-fr-sign`（考試人簽名／授課講師簽名兩條空白手簽線＋確認語），兩列印模板＋兩套 CSS 同步；不預印姓名；只在列印/分享頁生效，正常畫面零影響。build 20260625、commit `13e6e87` 已上線 |
