...
## 本輪完成（2026-05-29 第二十五次）✅ 本機 316/316 通過 — 零失敗

### KB-1 收尾：xinshing 欄 DM/DP/WI/FR 代號全量移除 + 關鍵字自然語言補強 ✅

**改動摘要：**

| 項目 | 狀態 |
|------|------|
| 全量掃描 xinshing 欄：128 處含文件編號代號 → 全部改為純中文描述（品質手冊/程序書/作業指導書/表單） | ✅ |
| 批次自動修正 54 處 `依，` → `依規定，` | ✅ |
| 批次自動修正 3 處 `（依）` → `（依規定）` | ✅ |
| 批次自動修正 1 處 `依及，` → `依規定，` | ✅ |
| 批次自動修正 1 處 `登錄於並列入` → `登錄後列入` | ✅ |
| 手動補齊 ~25 處語意殘渣（`記錄於；`→`記錄於該表單；` `填於。`→`填於查核表中。`等）| ✅ |
| verify_facts_ghpages.mjs expectHint 31 處同步改為中文表單名稱 | ✅ |
| fact-outsourcing-grade 補關鍵字（`委外合格分數` `幾分才算合格` `委外廠商要幾分` 等 15 個） | ✅ |
| fact-outsourcing-eval-timing 補關鍵字（`物流要怎麼評鑑` `委外物流評鑑` `委外要怎麼評鑑` 等 8 個） | ✅ |

**智慧查詢驗收：**
| 查詢 | 結果 |
|------|------|
| 「委外物流 要怎麼評鑑，合格分數為多少」 | #1 eval-timing（36分）+ #2 grade（36分）+ #3 clarify（10分）✅ |
| 「委外廠商要幾分才算合格」 | #1 grade（92分）✅ |
| 「委外怎麼評鑑」 | #1 eval-timing（241分）✅ |
| 「委外廠商60分怎麼辦」 | #1 grade（64分）✅ |

**驗收：** 本機 316/316 全部通過 ✅，零失敗

### 本輪 root cause

1. **xinshing 欄代號違規（§39.5）：** 建立 fact 時為求快速說明文件層級，直接寫入 DM/DP/WI/FR 代號。正確做法：一律用「品質手冊」「程序書」「作業指導書」「表單」純中文描述。
2. **批次修改後遺留語意殘渣：** 代號移除後留下一堆 `依，` `（依）` `記錄於；` 等不完整文法，需逐筆手動修復。
3. **自然語言關鍵字連續性問題：** `要` `怎麼` `才` `算` 等停用詞中斷 4 字連續匹配 → 需補各片段組合關鍵字。
4. **同一個 fact 需三種關鍵字型態：** 短代碼（FR72-02）+ 中文字尾碼（委外作業廠商評鑑表）+ 自然問法（物流要怎麼評鑑）。

### 本輪踩坑補記

| 踩坑 | 規則 |
|------|------|
| `依，` `（依）` 在代號移除後留下孤立 `依` → 需補 `依規定` | 代號刪除後要檢查上下文文法是否完整 |
| `記錄於；` `填於。` 等動詞後遺失受詞 | 代號移除後，動詞後需補明確表單名稱 |
| 長句查詢中 `要` `怎麼` 中斷 4 字連續匹配 | 需補中斷兩側的片段組合 + 完整長句 keyword |

### GitHub Pages 狀態

commit `90d1b2c` 已 push，永久網址已更新。
