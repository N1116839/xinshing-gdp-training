...
## 本輪完成（2026-06-02 第五十四次）✅ 建立視覺、合規與權限改善報告

- **使用者要求**：將目前 HTML 視覺化、規範違反、登入/權限規劃整理成可交給下一個 agent 處理的完整報告，並列入專案內。
- **新增文件**：`HTML視覺權限改善報告_2026-06-02.md`
- **報告重點**：
  1. 前台仍有技術字眼與開發者提示（Firebase / Firestore / 專案資料夾同步）需移除或改成學員可理解文案。
  2. 年度行事曆頁不應顯示 Firebase 個人專案、rules、部署指令等管理資訊。
  3. Q&A placeholder、導讀、pills 與部分前台主視覺仍直接露出 DM/DP/WI/FR 代碼，需收斂成中文概念或登入後調閱指引。
  4. 「規範到現場：四層對照」標題語意不精準，建議改為「規範到現場與公開缺失」或拆為三層對照 + 政府公開常見缺失。
  5. 常見缺失卡片需補逐項來源年份 / 來源文件 / 來源類型；無來源者暫不前台顯示。
  6. 掃描 174 筆 `docs.push` 條目，目前全部缺 `departments` / `allowedRoles`，登入功能前必須先補資料層權限標籤。
  7. Firestore rules 目前偏公開；正式登入後需改成 authenticated + role/profile 驗證，不可只靠前端隱藏。
- **本輪性質**：純文件與交接整理，未修改 HTML 功能。

## 下次開工提醒

1. 依 `HTML視覺權限改善報告_2026-06-02.md` 先修 learner-facing 技術字眼與文件代碼高風險顯示。
2. 常見缺失補來源欄位，無來源者暫不顯示。
3. 文件閱讀地圖重設計為「我想了解什麼 → 應調閱什麼 → 調閱目的」。
4. Phase 2 H1 員工登入系統前，先補 174 筆 KB 權限標籤。
5. 更新 Firestore rules 前需先完成角色/部門權限表。
