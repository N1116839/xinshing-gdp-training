...
> 使用者列印預覽回報兩項：FR24-03 內容距紙張邊界過近，且答題紀錄結尾有多張空白頁。已同步修正主頁列印 CSS 與分享列印 `examPacketCss()`：`@page` 從 `14mm 12mm 16mm` 調為上／下 `16mm`、左／右 `15mm`；FR24-03 表格單列高度 `13mm→10.5mm`、cell padding `6×7px→3×4px`，保留表尾簽名空間。原 `.exam-print-page` 的 `min-height:267mm + break-after:page` 會在自然跨頁後再產生空頁，改為無強制尾端換頁，僅用 `.exam-print-page + .exam-print-page{break-before:page}` 讓 FR24-03 與答題紀錄兩份文件分開；FR24-03 最小可用高度改為 `265mm`。`APP_BUILD`／`app-version.json` 已同步由 `20260728` 升為 `20260729`。
> 使用者回報人事列印資料包的訓練項目未出現年度缺失。根因：年度官方缺失題已從 `gdpGovExamQuestions` 抽入測驗，但 `tpl.printItems` 僅列共同基礎與 SOP 訓練。已新增 `recordTrainingItems()`：只有「年度教育訓練測驗」且實際抽到政府公開題時，才加一筆 `年度政府公開 GDP 常見缺失訓練（依食藥署 111、113 年度公開資料）`；年度由該次實際題目的 `sourceTitle/sourceSection` 自動帶出，沒有年度的 PIC/S 題則標為「官方公開資料」。本機列印 FR24-03、手機分享列印資料與結果頁人事列印資料包皆改用此清單；新人三個月測驗不新增。`APP_BUILD`／`app-version.json` 已同步由 `20260727` 升為 `20260728`。
> 使用者指定考後列印的「歷史紀錄」必須依 `第二章人事/FR24-03 員工教育訓練記錄表 文管.docx` 編排。已逐格核對原表的 8 欄底層格線與合併規則：資料區正式欄位為五欄「訓練項目／訓練單位／訓練者／訓練日期／人事簽核」，**原表沒有「評核成效」欄**。已同步修正 `HTML資料庫/新勝GDP資料庫.html` 的本機列印 `examPrintDocumentHtml()` 與手機分享列印 `sharedExamPrintDoc()`：移除非原表的評核成效欄與該欄成績，改採原表 8 欄比例的 `colgroup`／`colspan`，並將空白訓練列補足為原表的 8 列；測驗分數、合格結果仍保留在第二頁「答題紀錄」，不混入 FR24-03。表尾保留使用者指定的「考試人簽名」與「授課講師簽名」親簽線，兩個列印入口均已存在且同步驗證。`APP_BUILD`／根目錄 `app-version.json` 已同步由 `20260726` 升為 `20260727`。驗收：HTML JS parse 通過，FR24-03 五欄、兩入口、簽名欄與版本號同步檢查通過；`node verify_facts_ghpages.mjs` = `1306/1306`。後續：commit + push 後，以分享列印網址帶多筆訓練項目做瀏覽器列印預覽，確認第一頁 FR24-03 的表格與表尾簽名不被切頁。

> 2026-07-26（測驗列印、職稱與職級題庫修正 ✅，Firebase 已同步／rules 已部署／GitHub Pages 已推送）。
> 使用者提供四張截圖，確認同一列印問題：考卷答題紀錄跨頁時，每頁首題因 `100vh` 無頁首安全距離而被裁切；FR24-03 訓練項目較多時，固定在 A4 底部的簽名欄壓到表格。已修 `HTML資料庫/新勝GDP資料庫.html` 的本機列印及分享列印兩套 CSS：`@page` 改固定 A4 邊界 `14mm 12mm 16mm`、每頁採 `267mm` 可用高度；FR24-03 不再強制固定高度，簽名欄與文件編號改依表格後自然排版並禁止在元件內斷頁，避免重疊。測驗人員資料的主職與兼任／代理職務都新增「倉庫組長、業務組長、業助」。依 WI10-01 人事／關鍵人員資格與訓練要求，拆出 `warehouse_staff`、`warehouse_lead`、`sales_staff`、`sales_lead`、`sales_assistant` 五種職稱鍵：同一職系維持必訓 SOP 聯集；組長額外至少抽一題監督、複核或異常追蹤題。新增 Firestore `gdpGovExamQuestions`：18 題由已管理的 TFDA 111／113 官方公開缺失來源建立，完整涵蓋文管、人事、採購、管理藥師、品管、倉管、倉庫組長、業務、業務組長、業助，依 staff／leader 與職稱鍵分層；年度測驗會優先從 Firebase 取對應職級題，本機既有年度題仍為離線備援。`firestore.rules` 已成功部署、同步腳本已寫入 3 來源／60 缺失項／18 年度題，公開 REST 讀取核對為 18 題，10 種職稱鍵及每題的年度來源、題目、四選項與答案欄位皆齊全。驗收：`node verify_facts_ghpages.mjs` = `1306/1306`、HTML JS parse 與同步腳本 syntax 通過。`APP_BUILD`／`app-version.json` 已由 `20260723` 升為 `20260726`，推送後會要求已開頁面重整。待完成：commit + push 後，使用 GitHub Pages 實測紙本列印（含多訓練項目）與五職稱抽題。
...
> 第140次：使用者要求「幫 Claude 檢查，檢查後收工」，指定對應文件 `DP12-04 變更管制作業程序書`。已依開工規則讀 HANDOFF／踩坑／專案 note／GDP 規範／視覺權限報告／測驗規範，並依 `html-evidence-training-builder` 回來源核對。現況核對：本機 HEAD 已等於 `origin/codex/gdp-html-training-pages`（commit `f5dc188` 第139次已存在且已推送），但工作樹仍有 HTML 3 行未提交差異。逐字讀取 `第一章品質管理/DP12-04 變更管制作業程序書.docx`，確認未提交 3 行內容均有 SOP 原文依據：①「品質管理人或負責人等異動皆屬重大變更」②人事變更需新任人員學經歷資格審查、GDP 法規與相關 SOP 職責教育訓練，依 DP24-01 辦理 ③更新職務代理人名冊、簽名樣張與系統權限 ④涉及法規變更登記（如品質管理人異動）由 GDP 權責主管通報主管機關。HTML 補強內容：人事頁稽核問答 `evidenceToPrepare/answerDirection` 補資格審查、教育訓練合格、代理人名冊、簽名樣張、系統權限；`fact-keyperson-change` 新勝做法改成更貼近 DP12-04 原文；`fact-change-level` 重大變更範例補「品質管理人、負責人等關鍵人員異動」。驗收：抽 script 語法 `JS_PARSE_OK 1`；`node verify_facts_ghpages.mjs` = `1306/1306`、自動 keyword `838/838`、合規掃描通過。`app-version.json` 與 HTML `APP_BUILD` 均為 `20260627`，本輪只補內容精準度，不再 bump build。仍提醒：DP12-04 正文已改但制/修訂紀錄表仍停在版次 2／112-11-08，需使用者文件端補升版與修訂紀錄。
> 最新補記：2026-06-26（第139次：**使用者已修訂 DP12-04 把「人事變更」正式納入變更管制，平台 KB 同步更新 ✅**，build 踢人 `20260626→20260627`）。
> 第139次：開工讀完必讀文件後，使用者告知「**我已更新 DP12-04 變更管制作業程序書**」。⚠️ 依規範鐵則先回 SOP 原文逐字核對（unzip docx→PowerShell 抽正文）：**確認 DP12-04 內文現在已新增「人事變更」**——①範圍新增「人事變更：GDP關鍵職務(GDP權責人、品管、各級主管或關鍵作業人員)發生異動、離調職、組織架構重整或職務調整時之管制作業」②變更時機新增「人事變更時機」③一律填 FR12-05 變更管制需求單、評估變更等級（重大/次要/駁回）、品保評估追蹤、GDP權責主管核准結案。**＝第138次留給使用者的 §6.7 決策（DP12-04 是否納入人事變更）已正式拍板「納入」**，§6.7 落差關閉，依 §12（SOP 原文為準）KB 同步更新。AskUserQuestion 鎖範圍：①fact＋稽核問答都更新 ②新增一題人事變更測驗題。實作（`HTML資料庫/新勝GDP資料庫.html`）：①`fact-keyperson-change` xinshing 加「依 DP12-04 啟動人事變更管制：填 FR12-05、評估變更等級、品保追蹤、GDP權責主管核准結案」為①步並renumber，`sop_ref` 補 `DP12-04`/`FR12-05`（變首兩項），keyword 加人事變更專屬 3 詞（人事變更管制／關鍵人員變更要填什麼單／關鍵人員變更走變更管制）②人事頁稽核問答（第1302行 TFDA2024）text/evidence/auditQuestion/answerDirection 全補變更管制與 FR12-05 ③人事試卷 DP22-01 題組末新增測驗題（source DP12-04、topic 人事變更管制、options[0]正解「啟動人事變更管制，填寫 FR12-05…並評估變更等級」、含 explanation/sourceSection/sourceQuote 引 DP12-04 原文、reviewedBy「Claude 逐題回 DP12-04 原文核對」reviewDate 2026-06-26）。驗收：`JS_PARSE_OK 1`、`verify_facts 1306/1306`（fact 仍123、自動 keyword 838/838、合規通過、變更管制相關 fact 零回歸——新人事 keyword 未搶既有題）；preview 重載後服務頁原始碼確認五項新內容全上線＋APP_BUILD 20260627＋console 零 error。版本踢人 build `20260626→20260627`。**⚠️ 提醒使用者的文管追溯問題**：DP12-04 內文已含人事變更，但**制/修訂紀錄表仍停在「版次2 / 112/11/08 / 核准人由GDP權責人審核」，未新增一列記錄這次新增人事變更範圍**——依 DP42-01 受管制 SOP 內容變更應升版並登錄修訂紀錄（這份偏偏是變更管制 SOP 本身），是否補登版次屬使用者那邊文件作業，平台端未動。**下次待辦**：測驗正式題庫其餘職務逐題回 SOP 核對｜（可選）面板刪除帳號｜（可選）代理自動到期(Phase 3)。
> 最新補記：2026-06-26（第138次：**知識點新增——「GDP 關鍵人員變更（離職換人）的管理與申報」✅**，build 踢人 `20260625→20260626`）。
> 第138次：使用者製作 GDP 展延／後續檢查資料時，發現「GDP 關鍵人員變更也須走變更管制」，要我上網查規範並新增知識點，讓後續人員知道（哪個關鍵職位離職換人要做什麼、稽核怎麼查）。**⚠️ 關鍵查證結論（已讀公司 SOP 原文）：DP12-04 變更管制作業程序書（v2 112/11/08）範圍只有「文件變更＋工程變更」（＋偏差後），完全未把「人事／關鍵人員變更」列為變更管制觸發；DP22-01 組織與職掌也只用 FR12-01 任命書／FR22-01 指派名單／WI22-02 代理人管理，未寫要填 FR12-05。** 使用者發現的要求真正來源是「後續檢查／展延」端的外部明文：①國內藥商 GDP 後續檢查申請表附件明文要附「前次檢查至今之重大變更（含人事）清單」②PIC/S GDP 第二章要求維持最新組織圖／職務說明／代理人安排。＝規範 §6.7「外部有明文、公司 SOP 無明文」情境。**AskUserQuestion 裁示：新勝做法欄照現行 SOP 誠實寫（不宣稱填 FR12-05），範圍＝智慧查詢 fact＋稽核問答／常見缺失（不加測驗題庫）。** 實作（`HTML資料庫/新勝GDP資料庫.html`）：①新增 fact `fact-keyperson-change`（departments personnel/rp/qa/doc；international＝PIC/S Ch.2 組織圖/職務說明/代理人更新；taiwan＝後續檢查重大變更含人事申報＋藥商變更登記；xinshing＝權責主管換發 FR12-01 任命書／更新 FR22-01 指派名單＋WI22-01 職務說明書／新任者職前訓練合格才上線＋WI22-02 代理人／後續檢查列入重大變更清單申報；sop_ref 不含 DP12-04/FR12-05）②人事頁 focus[] 加一條 TFDA 2024 稽核問答（關鍵人員換人後文件未更新/未申報＝缺失；auditQuestion＝「上次檢查到現在 GDP 權責人或關鍵職位有換人嗎？換人後更新了哪些文件、有沒有申報？」）。驗收：`JS_PARSE_OK 1`、`verify_facts 1306/1306`（fact 122→123、自動 keyword 838/838 含本 fact 自我命中且零回歸、合規掃描通過）；preview 重載後頁面原始碼確認含 fact-keyperson-change＋稽核問答（kbStore 為閉包變數未掛 window，排序改由 verify_facts 資料層驗證）。版本踢人 build `20260625→20260626`。**⚠️ 留給使用者的決策（§6.7 外部明文 vs SOP 落差）：DP12-04 是否要正式把「關鍵人員變更」納入變更管制流程（要填 FR12-05）＝需修 DP12-04 SOP 原文，屬公司決定；目前知識點只誠實呈現「更新任命書/指派名單＋後續檢查申報」，未宣稱要填變更管制單。下次待辦**：測驗正式題庫其餘職務逐題回 SOP 核對｜（可選）DP12-04 是否修訂納入人事變更｜（可選）面板刪除帳號｜（可選）代理自動到期(Phase 3)。
> 第137次：2026-06-26（第137次：**FR24-03 員工教育訓練紀錄表底部加「考試人簽名／授課講師簽名」欄 ✅**，用途＝列印後親簽證明本人應考；版本踢人 build `20260624→20260625`，commit `13e6e87` 已上線並用永久網址驗收）。
> 第137次：使用者要在「考完試後產生的 FR24-03 員工教育訓練紀錄表」加考試人與授課講師簽名欄。盤點：FR24-03 由**兩個列印模板**產生——`examPrintDocumentHtml`（本機列印，session.profile 取 p.*）與 `sharedExamPrintDoc`（手機「分享送交人事資料」連結頁，取 d.*），結構一致，原本只有逐列「人事簽核」欄＋備註，缺整份的考試人/講師簽名。作法：在備註與右下角 `.exam-doc-number`(FR24-03) 之間新增 `.exam-fr-sign` 區塊（一行確認語「本人確認以上教育訓練測驗確由本人親自完成：」＋考試人簽名/授課講師簽名兩條空白手簽線），**`position:absolute;bottom:22mm` 釘在頁面底部空白處**（使用者明確要「下方空白處」非緊貼備註）。**使用者裁示不預印姓名**（純空白線供手簽，故移除 sign-name span 與其 CSS）。CSS 兩套都補：print `@media`(約636行)＋`examPacketCss()`(約7076行)。⚠️ 關鍵理解：`.exam-print-doc` 在**螢幕上 display:none**，簽名欄樣式只在 @media print／分享頁生效＝**正常網站桌機/手機畫面完全不受影響**，FR24-03 是固定 A4(mm) 版面、桌機手機列印輸出一致不會跑版。驗收：`JS_PARSE_OK 1`、`verify_facts 1296/1296`（零題庫變動）、preview 實測簽名欄渲染正確＋載入/導覽測驗區 console 零錯誤＋examSourceLabel/versionGuard/測驗多選(7 checkbox) 全無回歸＋永久網址 curl 確認 build 20260625＋exam-fr-sign/考試人簽名/授課講師簽名標記已上線。**下次待辦**：測驗正式題庫其餘職務逐題回 SOP 核對｜（可選）面板刪除帳號｜（可選）代理自動到期(Phase 3)。
> 第136次：2026-06-23（第136次：**①題庫補「無代碼來源」：作答後「受測考題回顧」＋列印「答題紀錄」每題上方顯示來源，零題庫資料變動 ✅ ②確認 page③ 依部門過濾早已完成（第128次），實機驗證 6 職稱全正確、無需改碼 ✅**，版本踢人 build `20260623→20260624`）。
...
| 版本踢人 | 站台根目錄 `app-version.json`（`build`）＋HTML `APP_BUILD` 常數＋`versionGuard`。要強制全員重新整理（＝重新登入）＝兩處 build 一起 +1 再 push。**目前 build＝`20260627`** |
| 最新本輪修正（第139次） | 使用者已修訂 DP12-04 正式把「人事變更」納入變更管制（§6.7 落差關閉）。平台 KB 同步：`fact-keyperson-change` xinshing 加「依 DP12-04 啟動人事變更管制、填 FR12-05、評估變更等級、GDP權責主管核准」＋sop_ref 補 DP12-04/FR12-05；人事頁稽核問答同步補；人事試卷新增一題人事變更管制測驗題（source DP12-04）。`verify_facts 1306/1306` 零回歸。⚠️ DP12-04 制修訂紀錄表未升版登錄，提醒使用者補。build 20260627 |
| 前輪修正（第138次） | 知識點新增「GDP 關鍵人員變更（離職換人）的管理與申報」：新增 fact `fact-keyperson-change`＋人事頁 TFDA 2024 稽核問答。當時 DP12-04 尚未納入人事變更，誠實寫不宣稱填 FR12-05（第139次 SOP 修訂後已改）。build 20260626 |
| 前輪修正（第137次） | FR24-03 員工教育訓練紀錄表底部新增 `.exam-fr-sign`（考試人簽名／授課講師簽名兩條空白手簽線＋確認語），兩列印模板＋兩套 CSS 同步；不預印姓名；只在列印/分享頁生效，正常畫面零影響。build 20260625、commit `13e6e87` 已上線 |
| 前輪修正（第136次） | ①題庫補「無代碼來源」：作答後「受測考題回顧」＋列印「答題紀錄」每題上方顯示來源（根據 SOP 第X章／依據國際法規／依據食藥署 113 年常見缺失），新增 `examSourceLabel`，**零題庫資料變動** ②確認 page③ 依部門過濾第128次已完成，實機驗證 6 職稱全正確、無需改碼。已用版本 20260624 push |
...
> 2026-09-07（來源覆核：品質手冊、SMF、組織職務、溫度、關鍵設備、文件、進出貨、儲存、訂單配銷與新增 FR57-01 正航確校報告 ✅）
> 已逐份回原始文件核對並修正前台／AI 問答／行事曆／測驗題庫：正航系統改為 NT357 網路版，確校期間 2024/07/01–07/05，使用單位為 GDP 權責人、倉庫、業務、品保、人事、採購；每日 00:00 自動備份、異地保存至少五年、異常呈報後還原備份；新增兩題適用職務題。移除無來源的年度確效提醒；嘉里收貨時間統一為 15:00–16:00。驗收：HTML inline JS parse 通過、verify_facts_ghpages 1308/1308、keyword 840/840、合規掃描通過。已檢查並保留所有使用者來源檔，未建立暫存或下載檔。